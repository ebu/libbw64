...
TEST_CASE("rect_24bit_noriff") {
  REQUIRE_THROWS_AS(Bw64Reader("testfiles/rect_24bit_noriff.wav"),
                    std::runtime_error);
}

TEST_CASE("rect_24bit_nowave") {
  REQUIRE_THROWS_AS(Bw64Reader("testfiles/rect_24bit_nowave.wav"),
                    std::runtime_error);
}

TEST_CASE("rect_24bit_wrong_fmt_size") {
  REQUIRE_THROWS_AS(Bw64Reader("testfiles/rect_24bit_wrong_fmt_size.wav"),
                    std::runtime_error);
}
